    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [
                ...
                OilStatItem(label: "High", value: String(format: "$%.2f", stats.high), color: .green)
                OilStatItem(label: "Low", value: String(format: "$%.2f", stats.low), color: .red)
                OilStatItem(label: "Average", value: String(format: "$%.2f", stats.average), color: .blue)
                OilStatItem(label: "Volatility", value: String(format: "%.2f%%", stats.volatility), color: .purple)
            }
        }
        .padding()
        .background(
